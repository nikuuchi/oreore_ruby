...
primary		: literal
		...
		{COND_POP();}
		  ...
		  { 
			...
			ID id2 = internal_id();
			ID *tbl = ALLOC_N(ID, 2);
			ID *tbl2 = ALLOC_N(ID, 2);
			NODE *m = NEW_ARGS_AUX(0, 0);
			NODE *m2 = NEW_ARGS_AUX(0, 0);
			...
			if (nd_type($7) == NODE_MASGN) {
			    NODE *one = NEW_LIST(NEW_LIT(INT2FIX(1)));
			    NODE *zero = NEW_LIST(NEW_LIT(INT2FIX(0)));
			    m2->nd_next = block_append(
				NEW_IF(
				    NEW_NODE(NODE_AND,
					     NEW_CALL(NEW_CALL(NEW_DVAR(id2), rb_intern("length"), 0),
						      rb_intern("=="), one),
					     NEW_CALL(NEW_CALL(NEW_DVAR(id2), rb_intern("[]"), zero),
						      rb_intern("kind_of?"), NEW_LIST(NEW_LIT(rb_cArray))),
					     0),
				    NEW_DASGN_CURR(id2,
						   NEW_CALL(NEW_DVAR(id2), rb_intern("[]"), zero)),
				    0),
				node_assign($7, NEW_DVAR(id2)));

			    args2 = new_args(m2, 0, id2, 0, 0);
			}
			else {
			    if (nd_type($7) == NODE_LASGN ||
				nd_type($7) == NODE_DASGN ||
				nd_type($7) == NODE_DASGN_CURR) {
				$7->nd_value = NEW_DVAR(id2);
				m2->nd_plen = 1;
				m2->nd_next = $7;
				args2 = new_args(m2, 0, 0, 0, 0);
			    }
			    else {
			...
			scope2 = NEW_NODE(NODE_SCOPE, tbl2, $12, args2);
			scope = NEW_NODE(NODE_SCOPE, tbl, NEW_FOR(0,$9,scope2), args);
			tbl[0] = 1; tbl[1] = id;
			tbl2[0] = 1; tbl2[1] = id2;
			$$ = NEW_FOR(0, $5, scope);
			fixpos($$, $2);
